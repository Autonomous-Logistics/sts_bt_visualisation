...
# Modified work Copyright (C) 2019-2020 Sebastian Mueller, Nick Fiege
#
...
class DotWidget(Gtk.DrawingArea):
    ...
    __gsignals__ = {
        ...
        #'draw_cb': (GObject.SIGNAL_RUN_LAST, None, (str,)),
        #'colorUpdate_cb': (GObject.SIGNAL_RUN_LAST, None, (bt_color_update,)),
    ...
    ### update function slightly changed, so that it's a generic draw for all uses
    def update(self):
        self.queue_draw()
...
class DotWindow(Gtk.Window):
    ...
    reloadGraphMlService = ''

    def __init__(self, widget=None, width=1024, height=776):
        ...
        ### KIKASS
        ### FGN
        ### FREEZES OCCUR when using 
        #stress --cpu 4 --io 4 --vm 4 --vm-bytes 1024M --timeout 40s
        self.dotwidget = widget or DotWidget()
        ### NO FREEZES OCCUR when using 
        #stress --cpu 4 --io 4 --vm 4 --vm-bytes 1024M --timeout 40s
        #self.dotwidget = widget or XaxaWidget()
        ### ....


        self.dotwidget.connect("error", lambda e, m: self.error_dialog(m))
        #self.dotwidget.connect('draw_cb', lambda e, m: self.drawTree(m))
        #self.dotwidget.connect('colorUpdate_cb', lambda e, m: self.colorUpdate(m))
    ...
    def drawTree(self, message):
        self.dotwidget._set_dotcode(message, None, False)
        self.dotwidget.zoom_to_fit()

    def updateTree(self, message):
        ### crawl and interprete event messages
        for u in message.updateList:
            if(u.event == 0):
                self.dotwidget.graph.updateNodeBorderColor(u.target, lookup_color(u.value))
            elif(u.event == 1):
                self.dotwidget.graph.updateNodeTextColor(u.target, lookup_color(u.value))
        ### this is important, we dont want to call queue_draw outside of the main thread!!!
        GLib.idle_add(self.dotwidget.update)
